...
const string defaultStudentBasic = "./cmake-build-debug/code";
...
void parseArguments(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "e:s:t:fmqch")) != -1) {
        switch (c) {
            case 'e':
                if (studentBasic.size()) useage(argv[0]);
                studentBasic = optarg;
                break;
            case 's':
                if (standerBasic.size()) useage(argv[0]);
                standerBasic = optarg;
                break;
            case 't':
                if (traceFile.size()) useage(argv[0]);
                traceFile = optarg;
                break;
            case 'f':
                if (firstFail) useage(argv[0]);
                firstFail = true;
                break;
            case 'm':
                if (hideError) useage(argv[0]);
                hideError = true;
                break;
            case 'q':
                if (silent) useage(argv[0]);
                silent = true;
                break;
            case 'h':
                useage(argv[0]);
